...
class HyenaFilter(OptimModule):
    ...
    def forward(self, x, L, k=None, bias=None, *args, **kwargs):
        ...
        mps_workaround = self.mps_fallback #x.device == 'mps' and self.mps_fallback
        if mps_workaround:
            x = x.to('cpu')
            k = k.to('cpu')
            bias = bias.to('cpu') if bias is not None else None
        y = fftconv(x, k, bias)
...
if __name__ == "__main__":
    ...
    x = torch.randn(1, 1024, 512, requires_grad=True).to(device)
    y = layer(x)
